...
from .models import about, Route, BusDetails, Destination, Customer,Payment,Ticket_history,NewUser,Contact,Passenger
import razorpay
...
from .serializers import BusDetailsSerializer,ContactSerializer,AddBusDetailsSerializer,UserSerializer,RegisterSerializer,RootSerializer,DestinationSerializer,TicketHistorySerializer,CustomerSerializer,AboutSerializer,ChangePasswordSerializer,LoginSerializer,PassengerSerializer
from django.contrib.auth.models import User
...
class LoginView(APIView):
    ...
    def post(self, request,format=None):
        ...
        # user = NewUser.objects.get(username=user.username)
        login(request, user)
        return Response({'data': {'user': user.username,'User_Id':user.user_id},
...
class BusDetailsView(APIView):
    ...
    def post(self, request, format=None):
        print(request.data)
        source = Destination.objects.get(destination=request.data['source'])
        request.data['source'] = source.id
        destination = Destination.objects.get(destination=request.data['destination_one'])
        request.data['destination_one'] = destination.id
    ...
    def patch(self,request):
        print(request.data)
        bus = BusDetails.objects.get(id=request.data['bus'])
        bus.rem -= int(request.data['seats_booked'])
        print(bus)
        bus.save()
        return Response(bus.rem)
...
class TicketHistoryView(APIView):
    def get(self,request,pk=None, format=None):
        obj = Passenger.objects.all()
        # obj = Passenger.objects.get(user_id=request.user_id)
        serializer = PassengerSerializer(obj, many=True)
        return Response(serializer.data)
...
class BookingHistory(APIView):
    def get(self,request, id=None, format=None):
        ...
        obj=Passenger.objects.filter(User_id=user_id)
        serializer=PassengerSerializer(obj,many=True)
        return Response(serializer.data)
...
class CustomerView(APIView):
    ...
    def post(self,request,format=None):
        print("request.data", request.data, sep='\n')
        # a = request.data
        d = [j for i in request.data['cusdetails'] for j in i]
        lst = []
        for i in d:
            i['mobile_number'] = request.data['mobile']
            i['no_tkt'] = 1
            lst.append(i)
        #     Source=i['source']
        #     Destination=i['destination']
        #     UserId= request.data.get('user_id')
        #     BusId=request.data.get('Bus_id')
        # print(Source)
        # print("list Data:", lst, sep='\n')
        # 
        # 
        # Destination=lst[destination]
        # print(Source,"++++++++++",Destination)
        # Booking_Id=str(UserId)+str(BusId)+Source[0:3]+Destination[0:3]
        serializer =CustomerSerializer(data=lst,many=True)
        if serializer.is_valid():
            serializer.save()
            return Response({'msg': 'data created'}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class PassengerView(APIView):
    ...
    def post(self,request,format=None):
        ...
        lst = []
        for i in d:
            i['Phone_number'] = request.data['Phone_number']
            i['no_tkt'] = 1
            i['Date_of_journey'] = a['Date_of_journey']
            i['Bus_id'] = a['Bus_id']
            i['User_id'] = a['User_id']
            Source=i['source']
            Destination=i['destination']
            ...
            lst.append(i)
            obj=BusDetails.objects.get(id=BusId)
            obj.rem-=a['no_tkts']
            obj.save()
        
        # print(lst)
        serializer =PassengerSerializer(data=lst,many=True)
        if serializer.is_valid():
            serializer.save()
            return Response({'msg': 'data created'}, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def buslist(request):
   # print(request.session['route'])
    ...
    # buses = BusDetails.objects.filter(destination=destination,origin=origin)
    if request.method == "POST":
